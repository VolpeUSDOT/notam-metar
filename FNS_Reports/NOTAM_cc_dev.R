...
# either by service area or across the whole NAS
...
for(delay_target_x in max_delay_target){
  ...
  for(Region_x in region_sequence){
    ...
    demand_day_of_interest = ninetieth_day_NAS[1, 'date']
    ...
    this_day = orig_dt_hr %>% 
      ...
      group_by(yr_hour, date, dow, hour, month) %>%
    ...
    # staff model 90 (TODO: CHANGE THIS ALGORITHM) ----    
  ...
  write.csv(staff_models,
            file = file.path(output_dir, 
                             paste0('Staff_models_90th_', delay_target_x, 'min_target.csv')),
            row.names = F)

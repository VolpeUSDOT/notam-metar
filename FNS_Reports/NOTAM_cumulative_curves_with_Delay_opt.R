...
for(Region in unique(ninetieth_days$Region)){
  ...
  # save to PDF
  fn = paste0(Region, '_90th_NOTAM_Cumulative_Curves.pdf')
  ...
  for(d in index_vector){
    ...
    if(use_cumulative_curves_to_estimate_delay){
      for(do_model in 1:nrow(hourly_staff_models_df)){
        print(as.character(hourly_staff_models_df$name[do_model]))
        if(hourly_staff_models_df$model_type[do_model] == "local naive"){
          hourly_staff_model = as.vector(hourly_staff_models_df$staff_list[do_model][[1]][[1]])
        } else
        {
          hourly_staff_model = as.vector(orig_df[orig_df$hourly_bin_start >= bod & orig_df$hourly_bin_start < eod, as.character(hourly_staff_models_df$name[do_model])])
        }
  ...
  write.csv(staff_df, file = paste0(Region, '_NOTAM_Staff_Models_by_hour.csv'))

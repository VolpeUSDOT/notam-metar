...
# Count total NOTMAS and total rows at each FPA
# Count total NOTMAS and total rows at each Service Area

# kable(rbind(counts_1, counts_2, counts_3), format = 'simple') 

```

The original data set contains `r format(nrow(d), big.mark = ',')` NOTMAS, from `r min(d$Action.Date)` to `r max(d$Action.Date)` with `r format(length(unique(d$Reference.ID)), big.mark = ',')` unique NOTAM Reference IDs.
...
With separate lines for weekend and weekday:
...
for(x in unique(df_Region$Region)){
  ...
  g2 <- ggplot(df_x, aes(count, fill = weekend, group = weekend)) +      # Within panels, group by weekend
    ...
    theme_bw() +
    ylab("Count of days") + xlab("Count of NOTAMs") +
    ggtitle(paste0(x, " Service Area: Histograms of NOTAM counts by day"))

...
for (i in 1:length(regions)) {
  ...
  cumul_model <- dt_busy_day %>%
    mutate(hour = minutes_elapsed / minutes_in_hour, .before = 1) %>%
    select(hour, cumulative_notams)

  incom_model <- dt_busy_day %>%
    mutate(hour = minutes_elapsed %/% minutes_in_hour, .before = 1) %>%
    group_by(hour) %>%
    summarize(incoming_notams = sum(incoming_notams))

  p <- ggplot() + xlab("Zulu time (hours)")
  # plot cumulative NOTAMs for the day
  pl <- p +
    geom_step(data = cumul_model,
              colour = c,
  ...
  pr <- p +
    geom_col(data = incom_model,
             colour = '#111111',
             aes(hour, incoming_notams),
             fill = c,
             size = .5) +
  ...
  for (i in 1:length(seasons)) {
    ...
    per_hour_seasons <- rbind(per_hour_seasons,
                              dt_busy_day %>%
                                group_by(season, hour = floor(hour)) %>%
                                summarize(incoming_notams =
                                            sum(incoming_notams))
    )
  }
